...
| 可配置热键 | 让用户避开与已有快捷键（如 Ctrl+反引号）的冲突。 |
...
3. 点击目标输入框，确保光标在其中。
4. 按住 <kbd>Ctrl</kbd> + <kbd>`</kbd>。
5. 说话。
6. 松开 <kbd>Ctrl</kbd> + <kbd>`</kbd>。
...
| `channels` | `1` | 录音通道数。 |
| `push_to_talk_hotkey` | `ctrl+\`` | 按住此热键开始录音。 |
...
Q：为什么热键不再打开 Codex / VS Code 终端了？
A：从 v0.3.2 开始，热键会在到达焦点应用之前被**拦截**。这可以防止应用级快捷键与按键说话冲突。如果你希望恢复旧行为，可以在 `config.json` 中换一个更不常用的组合键。

Q：Ctrl+反引号在 Codex 或其他应用里没反应，为什么？
A：某些应用会把 Ctrl+反引号保留给自己的终端或命令面板。即使进行了拦截，如果应用在更低层面钩住了键盘，全局监听器也可能收不到事件。你可以在 `config.json` 中修改 `push_to_talk_hotkey`，例如改成 `ctrl+shift+space`，然后重启 `run.bat`。
...
- 添加按应用区分的热键配置方案，例如针对已使用 Ctrl+反引号的 Codex 等应用。
